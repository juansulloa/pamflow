...
graphical_soundscape <- function(df, spec_wl=256, fpeaks_th=0.001, fpeaks_f=0, target_fs=48000, verbose=F){
  ...
  for(idx in 1:nrow(df)){
    ...
    s = readWave(fname)
    if (df$sample_rate[idx] > target_fs){
      s = resamp(s, g=target_fs, output = 'Wave')}

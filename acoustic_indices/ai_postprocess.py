...
df_mean = df.drop(columns=['fname', 'date', 'hour']).groupby(['sensor_name', 'period', 'day_period']).median().reset_index()
...
sns.boxplot(data=df_mean.groupby('day_period').get_group('night'), x='sensor_name', y='SC_MF', hue='period')